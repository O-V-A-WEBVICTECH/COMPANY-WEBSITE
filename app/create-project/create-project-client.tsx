...
import {
  ArrowLeft, Mail, Loader2, Send, CheckCircle2,
  Globe, Server, Smartphone, Code2,
} from "lucide-react";
...
export default function CreateProjectClient() {
  ...
  useEffect(() => {
    ...
    const tzIsNigerian = tz.startsWith("Africa/Lagos") || tz.startsWith("Africa/Abuja");
  ...
  const estimatedCost = useMemo(() => {
    ...
    if (isWebsite) {
      if (formData.domain) { const d = quotePricing.website.optional.find((x) => x.value === "domain"); if (d) { min += d.min; max += d.max; } }
      if (formData.hosting) { const h = quotePricing.website.optional.find((x) => x.value === "hosting_first"); if (h) { min += h.min; max += h.max; } }
    } else {
      if (formData.deployment) { const d = quotePricing.backend.optional.find((x) => x.value === "deployment"); if (d) { min += d.min; max += d.max; } }
      if (formData.ciCd) { const c = quotePricing.backend.optional.find((x) => x.value === "ci_cd"); if (c) { min += c.min; max += c.max; } }
    }
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <section className="bg-white border border-[#efeded] rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {([
                { cat: "website" as const, label: "Website / App", sub: "Frontend & full-stack", Icon: Globe },
                { cat: "backend" as const, label: "Backend / API", sub: "APIs, microservices & DBs", Icon: Server },
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="tel" required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className={inputClass} placeholder="+234 800 000 0000" />
                </div>
              </div>
            </div>
          </section>
          ...
          <section className="bg-white border border-[#efeded] rounded-xl p-5">
            ...
            <div className="flex flex-col gap-2">
              {isWebsite ? (
                <>
                  {[
                    { key: "domain" as const, label: "Domain Registration", note: `${fmt(15000, currency, exchangeRate)} / year` },
                    { key: "hosting" as const, label: "Web Hosting", note: `${fmt(10000, currency, exchangeRate)} first month` },
                  ].map(({ key, label, note }) => (
                    <label
                      key={key}
                ...
                <>
                  {quotePricing.backend.optional.map(({ value, label, min, max }) => {
                    const key = value === "deployment" ? "deployment" : "ciCd";
            ...
            <div className="flex flex-col gap-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.budget} onChange={(e) => setFormData({ ...formData, budget: e.target.value })} className={inputClass} placeholder="₦200,000 – ₦500,000" />
                </div>
                <div>
                  <label className={labelClass}>Timeline</label>
                  <input type="text" value={formData.timeline} onChange={(e) => setFormData({ ...formData, timeline: e.target.value })} className={inputClass} placeholder="2–3 months" />
                </div>
              </div>
              <div>
                <label className={labelClass}>Description</label>
                <textarea rows={4} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className={`${inputClass} resize-none`} placeholder="Tell us about your project, goals, and any specific requirements..." />
              </div>
            </div>
          </section>
