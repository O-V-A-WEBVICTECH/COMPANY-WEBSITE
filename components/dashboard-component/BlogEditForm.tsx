...
export default function BlogEditForm({ blogPost, onClose, onUpdate }: BlogEditFormProps): JSX.Element {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(blogPost.img || null);
  ...
  async function updateBlogPost(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.status === 200) { toast.success("Post updated"); onUpdate(); onClose(); }
  ...
  return (
    <form onSubmit={updateBlogPost} className="space-y-5">
      <div className="space-y-1.5">
        <Label htmlFor="be-title" className="text-sm font-semibold text-slate-700">
          Title <span className="text-red-500">*</span>
        </Label>
        <Input id="be-title" value={formData.title} onChange={(e) => handleInputChange("title", e.target.value)} required
          className="h-10 border-slate-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
      </div>

      <div className="space-y-1.5">
        <Label htmlFor="be-content" className="text-sm font-semibold text-slate-700">
          Content <span className="text-red-500">*</span>
        </Label>
        <Textarea id="be-content" value={formData.content} onChange={(e) => handleInputChange("content", e.target.value)}
          placeholder="Write your blog post content here..." rows={5} required
      ...
      <div className="space-y-1.5">
        ...
        <input ref={fileRef} type="file" accept="image/*" className="hidden" id="be-img-file" onChange={handleFileChange} />
        {imagePreview ? (
      ...
      <div className="space-y-1.5">
        <Label htmlFor="be-author" className="text-sm font-semibold text-slate-700">Author</Label>
        <Input id="be-author" value={formData.author} onChange={(e) => handleInputChange("author", e.target.value)}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <Label htmlFor="be-status" className="text-sm font-semibold text-slate-700">Status</Label>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="be-date" className="text-sm font-semibold text-slate-700">
            Date <span className="text-red-500">*</span>
          </Label>
          <Input id="be-date" type="date" value={formData.date} onChange={(e) => handleInputChange("date", e.target.value)} required
            className="h-10 border-slate-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" />
        </div>
      </div>
